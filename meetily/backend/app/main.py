...
import httpx
from transcription_providers import FasterWhisperServerClient
from transcription_providers import config as provider_config
...
# ---------------------------------------------------------------------------
# Transcription provider integration (Phase 1: faster-whisper-server)
# ---------------------------------------------------------------------------
#
# These endpoints implement the backend half of the integration plan:
#   - GET  /transcription-config                                   → load
#   - POST /transcription-config                                   → save
#   - GET  /transcription-providers/faster-whisper-server/health   → probe
#   - POST /transcribe-audio                                       → upload+go
#
# The Tauri frontend talks to *this* backend, never directly to faster-whisper-
# server, so the architecture stays installer-friendly (one stable API surface
# for the UI; the backend is free to swap providers underneath).


class TranscriptionConfigRequest(BaseModel):
    provider: str = provider_config.DEFAULT_PROVIDER
    serverUrl: str = provider_config.DEFAULT_SERVER_URL
...
async def get_transcription_config():
    """Return the active transcription provider config (HTTP-mediated providers)."""
    try:
        return provider_config.load_config()
    except Exception as exc:
        logger.error("Failed to load transcription config: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=str(exc))


@app.post("/transcription-config")
async def save_transcription_config(payload: TranscriptionConfigRequest):
    """Persist the active transcription provider config."""
    try:
        return provider_config.save_config(
...
@app.get("/transcription-providers/faster-whisper-server/health")
async def faster_whisper_server_health(serverUrl: Optional[str] = None):
    """Probe a faster-whisper-server instance and report normalized health.

    `serverUrl` may be passed as a query parameter to test an arbitrary URL
    before saving it; otherwise the persisted config value is used.
    """
    cfg = provider_config.load_config()
    url = serverUrl or cfg["serverUrl"]
    model = cfg["model"]
    client = FasterWhisperServerClient(server_url=url, model=model)
...
async def transcribe_audio(
    ...
    """Upload an audio file, forward to the active provider, return normalized JSON."""
    cfg = provider_config.load_config()

    # Today only fasterWhisperServer is HTTP-mediated; localWhisper / parakeet
    # remain handled inside Tauri/Rust (see integration plan §4.3).
    if cfg["provider"] != "fasterWhisperServer":
        raise HTTPException(
            status_code=400,
            detail=(
                f"Provider '{cfg['provider']}' is handled inside the desktop "
                "app (Tauri) and is not available via this HTTP endpoint."
            ),
        )
    ...
    async with sem:
        ...
        try:
            ...
            logger.error(
                "faster-whisper-server returned %s for %s",
                exc.response.status_code,
                file.filename,
            )
            raise HTTPException(
                status_code=502,
                detail=f"Provider error ({exc.response.status_code}): {exc.response.text[:300]}",
            ) from exc
        except httpx.HTTPError as exc:
            logger.error("Network error calling faster-whisper-server: %s", exc)
            raise HTTPException(
                status_code=502,
                detail=f"Could not reach transcription server at {cfg['serverUrl']}: {exc}",
            ) from exc
